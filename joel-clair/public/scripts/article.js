...
(function(module) {
  ...
  Article.numWordsAll = () => {
    return Article.all.map(articleObject => articleObject.body.split(' ').length).reduce((a,b) => (a + b),0)
  ...
  Article.allAuthors = () => {
    return Article.all.map(articleObject => articleObject.author).reduce(function (all, current) {
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      ...
      let sum = Article.all.filter(articleObject => articleObject.author === author).map(articleObject => articleObject.body.split(' ').length).reduce((a,b) => a + b);
